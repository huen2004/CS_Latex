...
Asimismo, esta etapa contribuye al Aprovisionamiento de la infraestructura y configuración de la red: VMware operativamente al cumplimiento de 
...
Es importante destacar que, sobre el sistema anfitrión, fue necesario ejecutar ajustes en las interfaces virtuales con el propósito de 
...
en modo promiscuo, ya que la replicación del tráfico se realizó de forma nativa dentro del entorno virtualizado. La configuración 
...
componente consiste en llevar a cabo procesos de inspección profunda de paquetes (\textit{Deep Packet Inspection}, DPI) sobre el tráfico 
...
acceso a la máquina virtual destinada al \IDPS\ para ejecutar la instalación nativa del motor de detección y su posterior configuración 
...
Entre las configuraciones a efectuar, se definió la variable \texttt{\$HOME\_NET}, utilizada para especificar los segmentos de red 
...
En el caso de OPNsense, la instalación del agente Wazuh se realizó mediante un \textit{plugin}, por lo que únicamente es necesario 
...
\texttt{eve.json} y envíe los registros al \textit{wazuh-manager}, permitiendo posteriormente ejecutar procesos de análisis, 
...
basados en estándares reconocidos de seguridad, como el \NISTCSF\ SP 800-53~\cite{NIST80053r5}.
...
National Institute of Standards and Technology (\NISTCSF).
...
Posteriormente a la adecuación del mecanismo de respuesta activa en el firewall, fue necesario configurar el archivo 
\textit{ossec.conf} del \textit{Wazuh Manager}, responsable de administrar los eventos y registros generados por los 
diferentes agentes de la infraestructura. Esta configuración permite que, ante la detección de eventos asociados a las 
reglas personalizadas de Suricata, el agente de OPNsense identificado con el ID \textit{001} ejecute el comando 
\textit{firewall-dropopnsense-fw}, correspondiente a un script alojado en el propio firewall.

Mediante este mecanismo, Wazuh puede enviar instrucciones a OPNsense para bloquear automáticamente direcciones IP 
catalogadas como sospechosas durante un período inicial de 300 segundos. Las direcciones bloqueadas son almacenadas en una 
lista gestionada mediante un alias del firewall, la cual es consultada por OPNsense para aplicar las restricciones 
correspondientes. Asimismo, si una dirección IP reincide en actividades potencialmente maliciosas, el tiempo de bloqueo 
puede incrementarse progresivamente hasta alcanzar períodos de 30 minutos.

De igual forma, se definieron restricciones asociadas al nivel de severidad de las alertas que pueden desencadenar acciones 
de respuesta activa. Para ello, únicamente se consideraron eventos con un nivel de alerta igual o superior a 8, evitando 
que eventos informativos o de baja criticidad generen bloqueos innecesarios y reduciendo así la probabilidad de falsos 
positivos derivados de auditorías rutinarias, actividades de monitoreo o escaneos legítimos.

La Figura~\ref{fig:fig41} presenta la configuración realizada en el archivo \textit{ossec.conf} para habilitar el mecanismo de respuesta 
activa dentro del \textit{Wazuh Manager}.
...
Finalmente, para que Wazuh pudiera interpretar correctamente los eventos generados por Suricata y procesarlos durante las 
etapas de correlación y respuesta activa, fue necesario implementar un \textit{decoder} local personalizado. Dicho 
\textit{decoder} fue configurado para heredar las capacidades del decodificador JSON utilizado de forma nativa por Wazuh, 
permitiendo reconocer y extraer adecuadamente los campos contenidos en los registros enviados por Suricata.

La implementación de este componente resulta fundamental dentro del flujo de procesamiento de eventos, ya que garantiza que 
la información recibida sea interpretada correctamente antes de ser evaluada por las reglas de detección y los mecanismos 
de respuesta activa definidos previamente. De esta manera, se evita la pérdida de información relevante y se reducen 
posibles errores durante el análisis de los eventos de seguridad.

La Figura~\ref{fig:decoders} presenta la configuración del \textit{decoder} desarrollado para habilitar la decodificación y el 
procesamiento de los registros provenientes de Suricata dentro de la plataforma Wazuh.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6]{tablas-images/pmv/decoders.jpeg}
	\caption{Creación de decoders de Suricata para Wazuh}\label{fig:decoders}
\end{figure}
...
estándar \NISTCSF\ SP 800-53~\cite{NIST80053r5}, análisis forense, correlación de eventos y respuesta activa. Por consiguiente, se da 
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.3]{tablas-images/pmv/fig42.jpeg}
